<div>
    <div class="flex justify-between items-center">
        <div class="flex">
            @can('manage', $tournament)
                <div class="mr-2">
                    <x-secondary-button :disabled="$tournament->hasAllTeams()">
                        <span>{{ __('Create team') }}</span>
                    </x-secondary-button>
                </div>
                @if($tournament->canGenerateTeams())
                    <div class="flex items-center">
                        <x-primary-button wire:loading.attr="disabled" :disabled="$this->generationInProgress" type="button" wire:click="generate" id="button-generate-teams">
    ...
    @if($tournament->teams->isEmpty())
        <div class="text-gray-700 text-lg text-center">{{ __('No teams created yet') }}</div>
    @else
                    ...
                    <div class="px-4 py-3 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                        ...
                        @can('manage', $tournament)
                            <x-danger-button type="button" :disabled="$this->generationInProgress"
                                             wire:loading.attr="disabled" wire:target="delete('{{ $team->id }}')" wire:click="delete('{{ $team->id }}')"
